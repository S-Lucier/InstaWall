"""
Mask to Walls v10 - Dangling endpoint merging:
- Merges dangling endpoints (not connected to other segments) within 15px
- Only merges if endpoint is truly disconnected, leaves connected endpoints alone
...
def merge_dangling_endpoints(segments, merge_distance=15):
    """
    V10: Merge dangling endpoints that are close together.

    Only merges endpoints that are NOT already connected to another segment.
...
def main():
    ...
    all_segments = merge_dangling_endpoints(all_segments, merge_distance=15)
