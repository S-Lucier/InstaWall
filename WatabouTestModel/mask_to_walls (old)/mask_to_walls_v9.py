"""
Mask to Walls v9 - Fix for collinear segment merging:
- Collinear segments (same direction) now merge when close, not just perpendicular ones
- Increased centroid merge tolerance from 5px to 10px
...
def is_collinear(seg1, seg2, tolerance_deg=15):
    """Check if two segments are roughly collinear (same direction)."""
    angle1 = get_segment_angle(seg1)
    angle2 = get_segment_angle(seg2)
    diff = abs(angle1 - angle2)
    # Normalize to [0, pi) since direction can be opposite
    diff = diff % math.pi
    tolerance_rad = math.radians(tolerance_deg)
    # Collinear if angles are close to 0 or close to pi
    return diff < tolerance_rad or abs(diff - math.pi) < tolerance_rad
...
def smart_corner_extension(segments, params):
    ...
    for cluster in clusters:
        ...
        # Get only wall segments with their endpoint info
        wall_segs = [(seg, idx, ep) for seg, idx, ep in cluster_segs if seg['type'] == 'wall']

        # V9: Process all pairs of close endpoints in the cluster
        # This handles both 2-segment and larger clusters
        processed_pairs = set()

        for i, (seg1, idx1, ep1) in enumerate(wall_segs):
            for j, (seg2, idx2, ep2) in enumerate(wall_segs):
                if i >= j or idx1 == idx2:
                    continue

                # Skip if either endpoint already adjusted
                ...
                # Only process close pairs (within 15px)
                if dist > 15:
                    continue
                ...
                # V9: Handle collinear segments - extend to midpoint
                if is_collinear(seg1, seg2):
                    midpoint = ((p1[0] + p2[0]) / 2, (p1[1] + p2[1]) / 2)
                    adjustments[idx1][ep1] = midpoint
                    adjustments[idx2][ep2] = midpoint
                    collinear_merged += 1
                    processed_pairs.add(pair_key)
                    continue

        # For remaining unprocessed endpoints, use centroid if very close
        ...
        # V9: Increased tolerance from 5 to 10
        if max_dist <= 10:
            centroid = (
    ...
    print(f"  Merged {collinear_merged} collinear segment pairs")
