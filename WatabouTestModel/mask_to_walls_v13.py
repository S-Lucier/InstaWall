"""
Mask to Walls v13 - Bug fixes:
- Fixed: Merged door detection - when two door regions touch (diagonal or L-shaped),
  they are now properly split into separate doors instead of creating one long door
- Fixed: Increased filter_doors_crossing_walls max_distance from 8 to 12 (> wall_offset)
- All v12 features: perimeter walls, dangling wall merge, nearest wall door extension
"""

# Debug flag - set to True to enable verbose door tracing
DEBUG_DOORS = False
...
def debug_print_doors(stage_name, segments):
    """Print door information for debugging."""
    if not DEBUG_DOORS:
        return

    doors = [s for s in segments if s['type'] == 'door']
    print(f"\n  [DEBUG {stage_name}] {len(doors)} doors:")
    for i, d in enumerate(doors):
        dx = d['x2'] - d['x1']
        dy = d['y2'] - d['y1']
        length = math.sqrt(dx*dx + dy*dy)
        orientation = "HORIZ" if abs(dx) > abs(dy) else "VERT" if abs(dy) > abs(dx) else "DIAG"
        door_id = d.get('id', '?')
        print(f"    Door[{door_id}]: ({d['x1']},{d['y1']}) -> ({d['x2']},{d['y2']}) "
              f"len={length:.1f} {orientation}")
...
def extract_doors(mask, params):
    ...
    for idx, contour in enumerate(contours):
        ...
        # DEBUG: Show contour info
        if DEBUG_DOORS:
            print(f"    Contour {idx}: bbox=({x},{y},{w},{h}) center=({cx},{cy}) area={area:.0f}")

        # V13 FIX: Detect L-shaped merged doors (area ~2x normal, bbox roughly square and large)
        # Normal door is ~70x70 with area ~4761. L-shaped merge is ~140x140 with area ~9522
        normal_door_area = 70 * 70  # approximate
        is_likely_merged = (area > normal_door_area * 1.5 and
                           min(w, h) > 100 and
                           0.7 < w/h < 1.4)  # roughly square

        if is_likely_merged:
            if DEBUG_DOORS:
                print(f"      -> Detected likely merged doors, analyzing shape...")

            # Extract the actual door pixels from this contour region
            contour_mask = np.zeros_like(door_mask)
            cv2.drawContours(contour_mask, [contour], -1, 255, -1)

            # Check which quadrants have door pixels
            mid_x = x + w // 2
            mid_y = y + h // 2

            # Sample the four quadrants
            tl = contour_mask[y:mid_y, x:mid_x].sum()
            tr = contour_mask[y:mid_y, mid_x:x+w].sum()
            bl = contour_mask[mid_y:y+h, x:mid_x].sum()
            br = contour_mask[mid_y:y+h, mid_x:x+w].sum()

            if DEBUG_DOORS:
                print(f"      Quadrants: TL={tl}, TR={tr}, BL={bl}, BR={br}")

            # Threshold for "filled" quadrant
            threshold = max(tl, tr, bl, br) * 0.3
            ...
            if DEBUG_DOORS:
                print(f"      Filled quadrants: {[k for k,v in filled.items() if v]}")

            door_width_mult = params['door_width_mult']
            half_w = w // 2
            half_h = h // 2

            # Case 1: Diagonal pattern (TL+BR or TR+BL) - two separate doors
            if filled_count == 2:
                if filled['TL'] and filled['BR']:
                    # Two diagonally opposite doors
                    # TL door - determine orientation by checking surrounding walls
                    tl_cx, tl_cy = x + half_w // 2, y + half_h // 2
                    # BR door
                    br_cx, br_cy = x + w - half_w // 2, y + h - half_h // 2

                    # For each sub-door, determine orientation from walls
                    for sub_cx, sub_cy, sub_name in [(tl_cx, tl_cy, 'TL'), (br_cx, br_cy, 'BR')]:
                        search_radius = half_w + 10
                        sy1 = max(0, sub_cy - search_radius)
                        sy2 = min(mask.shape[0], sub_cy + search_radius)
                        sx1 = max(0, sub_cx - search_radius)
                        sx2 = min(mask.shape[1], sub_cx + search_radius)

                        region = wall_mask[sy1:sy2, sx1:sx2]
                        region_h, region_w = region.shape

                        # Check horizontal vs vertical wall presence
                        center_y = min(search_radius, region_h // 2)
                        center_x = min(search_radius, region_w // 2)

                        left_walls = region[center_y, :center_x].sum() if center_x > 0 else 0
                        right_walls = region[center_y, center_x:].sum() if center_x < region_w else 0
                        horiz_walls = left_walls + right_walls

                        top_walls = region[:center_y, center_x].sum() if center_y > 0 else 0
                        bottom_walls = region[center_y:, center_x].sum() if center_y < region_h else 0
                        vert_walls = top_walls + bottom_walls

                        if horiz_walls > vert_walls:
                            # Horizontal door
                            door_w = int(half_w * door_width_mult)
                            doors.append({
                                'id': door_id,
                                'x1': sub_cx - door_w // 2, 'y1': sub_cy,
                                'x2': sub_cx + door_w // 2, 'y2': sub_cy,
                                'type': 'door'
                            })
                        else:
                            # Vertical door
                            door_h = int(half_h * door_width_mult)
                            doors.append({
                                'id': door_id,
                                'x1': sub_cx, 'y1': sub_cy - door_h // 2,
                                'x2': sub_cx, 'y2': sub_cy + door_h // 2,
                                'type': 'door'
                            })
                        door_id += 1

                    if DEBUG_DOORS:
                        print(f"      Created 2 doors from diagonal pattern (TL+BR)")
                    continue

                elif filled['TR'] and filled['BL']:
                    # Two diagonally opposite doors (other diagonal)
                    tr_cx, tr_cy = x + w - half_w // 2, y + half_h // 2
                    bl_cx, bl_cy = x + half_w // 2, y + h - half_h // 2

                    for sub_cx, sub_cy, sub_name in [(tr_cx, tr_cy, 'TR'), (bl_cx, bl_cy, 'BL')]:
                        search_radius = half_w + 10
                        sy1 = max(0, sub_cy - search_radius)
                        sy2 = min(mask.shape[0], sub_cy + search_radius)
                        sx1 = max(0, sub_cx - search_radius)
                    ...
                    if DEBUG_DOORS:
                        print(f"      Created 2 doors from diagonal pattern (TR+BL)")
                    continue

            # Case 2: L-shape (3 quadrants filled) - fallback to original logic
            if filled_count >= 3:
                quadrants = [(tl, 'TL'), (tr, 'TR'), (bl, 'BL'), (br, 'BR')]
                quadrants.sort(key=lambda q: q[0])
                empty_quad = quadrants[0][1]

                if empty_quad == 'TL':
                    doors.append({'id': door_id, 'x1': mid_x, 'y1': y + half_h // 2, 'x2': x + w, 'y2': y + half_h // 2, 'type': 'door'})
                    door_id += 1
                    doors.append({'id': door_id, 'x1': x + half_w // 2, 'y1': mid_y, 'x2': x + half_w // 2, 'y2': y + h, 'type': 'door'})
                    door_id += 1
                elif empty_quad == 'TR':
                    doors.append({'id': door_id, 'x1': x, 'y1': y + half_h // 2, 'x2': mid_x, 'y2': y + half_h // 2, 'type': 'door'})
                    door_id += 1
                    doors.append({'id': door_id, 'x1': x + w - half_w // 2, 'y1': mid_y, 'x2': x + w - half_w // 2, 'y2': y + h, 'type': 'door'})
                    door_id += 1
                elif empty_quad == 'BL':
                    doors.append({'id': door_id, 'x1': mid_x, 'y1': y + h - half_h // 2, 'x2': x + w, 'y2': y + h - half_h // 2, 'type': 'door'})
                    door_id += 1
                    doors.append({'id': door_id, 'x1': x + half_w // 2, 'y1': y, 'x2': x + half_w // 2, 'y2': mid_y, 'type': 'door'})
                    door_id += 1
                else:  # BR
                    doors.append({'id': door_id, 'x1': x, 'y1': y + h - half_h // 2, 'x2': mid_x, 'y2': y + h - half_h // 2, 'type': 'door'})
                    door_id += 1
                    doors.append({'id': door_id, 'x1': x + w - half_w // 2, 'y1': y, 'x2': x + w - half_w // 2, 'y2': mid_y, 'type': 'door'})
                    door_id += 1

                if DEBUG_DOORS:
                    print(f"      Created 2 doors from L-shape (empty quadrant: {empty_quad})")
                continue

            # Fallback: treat as single large door (shouldn't normally happen)
            if DEBUG_DOORS:
                print(f"      Fallback: treating as single door")

        # Normal single door processing
...
def smart_corner_extension(segments, params):
    ...
    for cluster in clusters:
        ...
        door_segs = [(seg, idx, ep) for seg, idx, ep in cluster_segs if seg['type'] == 'door']

        # DEBUG: Check if doors are in this cluster
        if DEBUG_DOORS and door_segs:
            print(f"  [DEBUG CLUSTER] Found {len(door_segs)} door(s) in cluster with {len(wall_segs)} wall(s):")
            for seg, idx, ep in door_segs:
                door_id = seg.get('id', '?')
                pt = (seg['x1'], seg['y1']) if ep == 1 else (seg['x2'], seg['y2'])
                print(f"    Door[{door_id}] endpoint {ep} at {pt}")
            for seg, idx, ep in wall_segs:
                pt = (seg['x1'], seg['y1']) if ep == 1 else (seg['x2'], seg['y2'])
                print(f"    Wall seg {idx} endpoint {ep} at {pt}")
        ...
        if max_dist <= 5:  # Increased from 3 to 5
            ...
            # DEBUG: Warn if doors are being adjusted
            if DEBUG_DOORS and door_segs:
                door_ids = [seg.get('id', '?') for seg, idx, ep in door_segs]
                print(f"  [DEBUG WARNING] Adjusting Door[{door_ids}] endpoint(s) to centroid {centroid}!")
...
def filter_doors_crossing_walls(segments, max_distance=8):
    ...
    for i, door in enumerate(doors):
        ...
        if min_dist1 <= max_distance and min_dist2 <= max_distance:
            ...
            if DEBUG_DOORS:
                door_id = door.get('id', '?')
                print(f"  [DEBUG FILTER] Removing Door[{door_id}]: ({door['x1']},{door['y1']})->({door['x2']},{door['y2']})")
                print(f"    p1 dist to wall: {min_dist1:.1f}, p2 dist to wall: {min_dist2:.1f} (max allowed: {max_distance})")
...
def snap_doors_to_wall_endpoints(segments, snap_distance=5):
    ...
    for i, door in enumerate(doors):
        ...
        snap1 = dist1 <= snap_distance
        snap2 = dist2 <= snap_distance

        # DEBUG: Show what's being snapped
        if DEBUG_DOORS and (snap1 or snap2):
            door_id = door.get('id', '?')
            orig_dx = door['x2'] - door['x1']
            orig_dy = door['y2'] - door['y1']
            orig_orient = "HORIZ" if abs(orig_dx) > abs(orig_dy) else "VERT"
            print(f"  [DEBUG SNAP] Door[{door_id}] ({orig_orient}): ({door['x1']},{door['y1']})->({door['x2']},{door['y2']})")
            if snap1:
                target1 = wall_endpoints[idx1]
                print(f"    Snap p1 ({door['x1']},{door['y1']}) -> ({int(target1[0])},{int(target1[1])}) dist={dist1:.1f}")
            if snap2:
                target2 = wall_endpoints[idx2]
                print(f"    Snap p2 ({door['x2']},{door['y2']}) -> ({int(target2[0])},{int(target2[1])}) dist={dist2:.1f}")
        ...
        # DEBUG: Check if orientation changed
        if DEBUG_DOORS and (snap1 or snap2):
            new_dx = new_door['x2'] - new_door['x1']
            new_dy = new_door['y2'] - new_door['y1']
            new_orient = "HORIZ" if abs(new_dx) > abs(new_dy) else "VERT"
            if orig_orient != new_orient:
                print(f"    [WARNING] Door[{door_id}] orientation changed from {orig_orient} to {new_orient}!")
            new_len = math.sqrt(new_dx*new_dx + new_dy*new_dy)
            if new_len < 5:
                print(f"    [WARNING] Door[{door_id}] became very short: len={new_len:.1f}!")
...
def filter_short_segments(segments, min_length):
    ...
    for seg in segments:
        ...
        if length >= min_length:
            ...
            removed += 1
            if DEBUG_DOORS and seg['type'] == 'door':
                door_id = seg.get('id', '?')
                print(f"  [DEBUG SHORT] Removing short Door[{door_id}]: ({seg['x1']},{seg['y1']})->({seg['x2']},{seg['y2']}) len={length:.1f}")
...
def filter_zero_length_segments(segments):
    ...
    for seg in segments:
        if seg['x1'] == seg['x2'] and seg['y1'] == seg['y2']:
            ...
            if DEBUG_DOORS and seg['type'] == 'door':
                door_id = seg.get('id', '?')
                print(f"  [DEBUG ZERO] Removing zero-length Door[{door_id}] at ({seg['x1']},{seg['y1']})")
        else:
            filtered.append(seg)
...
def main():
    ...
    parser.add_argument("--output", type=str, default="wall_visualization_v13.png")
    ...
    debug_print_doors("AFTER EXTRACT", doors)
    ...
    debug_print_doors("AFTER STRAIGHTEN", all_segments)
    ...
    debug_print_doors("AFTER CORNER_EXT", all_segments)
    ...
    debug_print_doors("AFTER EXTEND", all_segments)

    # Step 7: V6 - Filter invalid doors BEFORE splitting (prevents orphan splits)
    # V13: Increased max_distance from 8 to 12 (must be > wall_offset of 10px)
    print("\nStep 7: Filtering doors that don't cross walls...")
    all_segments = filter_doors_crossing_walls(all_segments, max_distance=12)
    debug_print_doors("AFTER FILTER", all_segments)
    ...
    debug_print_doors("AFTER SPLIT", all_segments)
    ...
    debug_print_doors("AFTER SNAP", all_segments)
    ...
    debug_print_doors("AFTER CLEANUP", all_segments)
    ...
    debug_print_doors("AFTER MERGE", all_segments)
